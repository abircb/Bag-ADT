...
public interface Bag<T extends Comparable> extends Iterable<T>
{
  ...
  /**
   * The current working directory where the properties were initialized
   * final is used, so that it is fixed
   */
  final String dir = System.getProperty("user.dir") + System.lineSeparator();
  ...
  /**
   * This method removes all duplicates in a Bag.
   */
  ...
  /**
   * @param bag
   * @return a new Bag containing all values and occurrences that occur in the 
   * this Bag (Bag object calling this method) but not the argument Bag
   * @throws BagException
   */
  public Bag<T> subtract(Bag<T> bag) throws BagException;
  
  /**
   * This method writes the Bag data structure to a text-file with its 
   * type parameter as the heading of the file <br />
   * Note- works for all java primitive types and String
   * @param bag- The bag to subtract from this
   * @return The new bag 
   */
  public void toFile() throws IOException;
  
  /**
   * This method reads the text-file representation of a Bag and then makes a Bag object out of it<br />
   * Note- only works if the first line of the file specifies the type of the Bag
   * @param file
   * @return A bag of the type stored in the text file. Null, if file is incompatible 
   * (i.e) does not have type specified in the first line
   * @throws IOException
   * @throws BagException
   */
